...
parse_duration() {
	...
	([ "$minutes" -ge 60 ] || [ "$seconds" -ge 60 ]) && return 1
...
get_timer_remaining() {
	if ! is_timer_set; then return 1; fi	
	if is_timer_paused; then echo "$(get_timer_duration)"; return 0; fi
	elapsed="$(get_timer_elapsed)"
	duration="$(get_timer_duration)"
...
rofi_dmenu() {
	...
	if ! echo "$input" | grep -Eq '^(([0-9]{1,}:)?[0-5]?[0-9]:)?[0-5]?[0-9]$'; then exit 1; fi
	start_timer "$input"
}
