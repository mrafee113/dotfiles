...
gen_cheats() {
	[ ! "$1" = -f ] && [[ -f /tmp/cheat-files && -z "$(find -L "${HOME}/.config/cheat/cheatsheets" -type f -newer /tmp/cheat-files ! -path "*/.git/*" ! -path "*/.github/*")" ]] && return 0
	find -L "${HOME}/.config/cheat/cheatsheets" -type f ! -path "*/.git/*" ! -path "*/.github/*" ! -path "${HOME}/.config/cheat/cheatsheets/LICENSE" ! -path "${HOME}/.config/cheat/cheatsheets/README.md" |
